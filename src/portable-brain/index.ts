...
import { formatRankedLessons, rankLessons, type RankedLesson } from "../learning/lessons.js";

export const BRAIN_DIR = ".app-factory";
export const BRAIN_VERSION = 1 as const;

/** Ranked lesson persisted into the portable brain (full ranking metadata). */
export type PortableRankedLesson = RankedLesson;
...
export interface PortableBrainState {
  ...
  /** Full ranked lessons (id, scope, relevance, rankReason) — primary structured field. */
  lessonsRanked: PortableRankedLesson[];
  /** Same as lessonsRanked (compat for older readers). */
  lessons: PortableRankedLesson[];
  /** Agent-facing formatted lines, same shape as get_context.lessonsLearned. */
...
function buildState(project: store.Project, workspacePath: string, lastSyncedEventId: number): PortableBrainState {
  ...
  // Ranked so the most relevant lessons surface first for any outside agent.
  const lessonsRanked: PortableRankedLesson[] = rankLessons(
    store.getLessons(project.id),
    lessonContext,
    20,
  ).map((l) => ({
    id: l.id,
    ...
    rankReason: l.rankReason,
  }));
  const lessonsLearned = formatRankedLessons(lessonsRanked);
...
function renderRankedLessonsMd(lessons: PortableRankedLesson[], limit = 12): string {
  if (!lessons.length) return "_No ranked lessons yet. Call refine after hard fixes to capture them._";
  return lessons
    .slice(0, limit)
    .map(
      (l) =>
        `- **[${l.topic}|${l.scope}|rel=${l.relevance}|${l.rankReason}]** ${l.lesson}` +
        (l.evidence ? `\n  - evidence: ${l.evidence.slice(0, 200)}` : ""),
    )
    .join("\n");
}
...
function renderBrainMd(state: PortableBrainState): string {
  ...
  const problems = state.openProblems.length
    ...
    : "_No open problems._";
  const ranked = state.lessonsRanked?.length ? state.lessonsRanked : state.lessons;
  const lessonsMd = renderRankedLessonsMd(ranked ?? [], 20);
...
> **Ranked lessons below are binding rules** — obey them before inventing a new approach.
...
See \`.app-factory/state.json\` (\`lessonsRanked\`, \`lessonsLearned\`) and \`.app-factory/journal.jsonl\`.
`;
}

function renderAgentsMd(state: PortableBrainState): string {
  const ranked = state.lessonsRanked?.length ? state.lessonsRanked : state.lessons;
  const topLessons = renderRankedLessonsMd(ranked ?? [], 8);
...
1. Read \`.app-factory/BRAIN.md\` — narrative recap (goal, phase, next step, **ranked lessons**, decisions, problems).
2. Read \`.app-factory/state.json\` — machine-readable state (\`lessonsRanked\` has full ranking metadata).
...
## Ranked lessons (obey these)

${topLessons}

Full list + metadata: \`.app-factory/state.json\` → \`lessonsRanked\` / \`lessonsLearned\`.

## If App Factory MCP is available

Call \`get_context\` / \`get_project_state\` with projectId \`${state.projectId}\` (also returns \`lessonsRanked\`), then \`sync_portable_brain\` after meaningful progress.

## If App Factory MCP is NOT available

Continue from **What to do next** and **Ranked lessons** in \`.app-factory/BRAIN.md\`. When you make a decision or hit a problem, append a line to \`.app-factory/journal.jsonl\` (JSON object with \`kind\`, \`name\`, \`detail\`, \`createdAt\`, \`source: "agent"\`) and update \`BRAIN.md\` / \`open-problems.md\` / \`decisions.md\` accordingly. Never write secrets into the brain.
...
function renderClaudeMd(state: PortableBrainState): string {
  ...
  const lessonLines = top.length
    ? top.map((l) => `- [${l.topic}|rel=${l.relevance}] ${l.lesson}`).join("\n")
    : "- (none yet — see BRAIN.md after refine)";
  return `# ${state.name}

Claude Code / coding-agent entrypoint for this project.

**Read \`AGENTS.md\` and \`.app-factory/BRAIN.md\` before changing code. Obey ranked lessons.**

- Phase: ${state.phase}
- Next: ${state.nextStep}
- Structured state: \`.app-factory/state.json\` (\`lessonsRanked\`)
